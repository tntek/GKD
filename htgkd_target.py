...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 and args.cls_par > 0:
            netF.eval()
            netB.eval()
            mem_label_soft, mtx_infor_nh, feas_FC = obtain_label(dset_loaders['test'], netF, netB, netC, args, iter_num, iter_sw)
...
def obtain_label(loader, netF, netB, netC, args, num_iter_f, iter_sw_f):
    start_test = True
    with torch.no_grad():
        iter_test = iter(loader)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='HTGKD')
